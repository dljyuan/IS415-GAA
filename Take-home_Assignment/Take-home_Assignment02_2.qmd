...
```{r}
#dengue <- dengue[dengue["x-coordinate"] != "None", ]
#dengue_sf <- st_as_sf(dengue, coords = c("x-coordinate", "y-coordinate"), crs = 3824)
dengue <- dengue %>%
...
With these Gi\* measures we can then evaluate the locations highlighted in the PART 1 of the assignment for a trend using the Mann-Kendall test.
...
cbg <- gi_stars %>% 
  ungroup() %>% 
  filter(combined_location == "安南區-大安里") |> 
  select(combined_location, WEEK, gi_star)
```
